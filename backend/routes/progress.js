...
progressRouter.post("/dsa", requireAuth, async (req, res) => {
    try {
        ...
        const progress = await progressModel.create({ userId, date, type, hours, questions });
...
progressRouter.post("/development", requireAuth, async (req, res) => {
    try {
        ...
        const progress = await progressModel.create({ userId, date, type, hours });
        return res.status(201).json({ message: "Progress Added", progress });
    } catch (error) {
        return res.status(500).json({ message: "Something went wrong", error });
    }
...
progressRouter.get("/type/:type", requireAuth, async (req, res) => {
    try {
        ...
        const progress = await progressModel.find({ userId: req.userId, type }).sort({ date: -1 });
        return res.status(200).json({ progress });
    } catch (error) {
        return res.status(500).json({ message: "Progress not found", error });
    }
});
